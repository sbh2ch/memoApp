...
public class MultiMemoActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        newMemoBtn.setBackgroundBitmap(R.drawable.new_btn, R.drawable.new_btn_click);
        ...
        closeBtn.setBackgroundBitmap(R.drawable.cancel_btn1, R.drawable.cancel_btn_click);
    ...
    public void openDatabase() {
        // open database
        if (mDatabase != null) {
            Log.d("MemoDatabase","메인함수에서 mDb가 null이 아님. ");

            mDatabase.close();
            Log.d("MemoDatabase", "메인함수에서 mDatabase.close ");

            mDatabase = null;
        }

        Log.d("MemoDatabase","메인함수에서 getInstance호출 ");
        mDatabase = MemoDatabase.getInstance(this);
        Log.d("MemoDatabase","메인함수에서 getInstance호출 완료");

        boolean isOpen = mDatabase.open();
        Log.d("MemoDatabase", "mDatabase.open() 실행.");
        if (isOpen) {
            Log.d("MemoDatabase", "Memo database is open.");
        } else {
            Log.d("MemoDatabase", "Memo database is not open.");
        }
    }
